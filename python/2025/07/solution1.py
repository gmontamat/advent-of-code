...
# Pole research wing.
#
# Based on the large sign that says "teleporter hub", they seem to be researching teleportation; you can't help but try
# it for yourself and step onto the large yellow teleporter pad.
#
...
# location marked S; tachyon beams always move downward. Tachyon beams pass freely through empty space (.). However, if
# a tachyon beam encounters a splitter (^), the beam is stopped; instead, a new tachyon beam continues from the
...
# ...............
#
# At this point, the two splitters create a total of only three tachyon beams, since they are both dumping tachyons into
...
# |.|.|.|.|.|||.|
#
# To repair the teleporter, you first need to understand the beam-splitting properties of the tachyon manifold. In this
...
if __name__ == "__main__":
    ...
    # main loop
    splits = 0
    for prev_i, row in enumerate(manifold[1:]):
        for j, col in enumerate(row):
            if col == ".":
                if manifold[prev_i][j] in ["S", "|"]:
                    manifold[prev_i + 1][j] = "|"
