...
typedef struct {
    int32_t key;
    int32_t value;
} State;  // Maps state -> steps to get there
...
uint32_t computeState(int32_t *generator, int32_t *microchip) {
    // TODO
    // do not explore states that we have already been in
    return 0;
}
...
int32_t dfs(int32_t *generator, int32_t *microchip, size_t elements,
    ...
    // TODO: compute state so as not to backtrack!
    uint32_t state_id = computeState(generator, microchip);
    if (hmgeti(state, state_id) == -1 || state[state_id].value > steps) {
        hmput(state, state_id, steps);  // We got here in fewer steps
    } else {
        return min_steps;
    }
    // Go up
    if (floor < 4) {
        for (size_t i = 0; i < elements; ++i) {
            ...
            for (size_t j = 0; j < elements; ++j) {
                ...
                if (microchip[i] == floor && generator[j] == floor && i < j) {
                    bool move = true;  // Can we move both from this floor?
