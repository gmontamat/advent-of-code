...
int32_t dfs_minimum(int32_t liters, int32_t *containers, int32_t current, int32_t minimum) {
    for (size_t i=0; i<(size_t)arrlen(containers); ++i) {
        if (containers[i] < liters) {
            ...
            minimum = dfs_minimum(liters - containers[i], remaining, current + 1, minimum);
            arrfree(remaining);
        } else if (containers[i] == liters) {
            if (current + 1 < minimum) minimum = current + 1;
        }
    }
    return minimum;
}

int32_t dfs(int32_t liters, int32_t *containers, int32_t current, int32_t minimum) {
    int32_t ways = 0;
    for (size_t i=0; i<(size_t)arrlen(containers); ++i) {
        if (containers[i] < liters) {
            int32_t *remaining = NULL;
            ...
            ways += dfs(liters - containers[i], remaining, current + 1, minimum);
            arrfree(remaining);
        } else if (containers[i] == liters) {
            if (current + 1 == minimum) ways++;
        }
    }
    return ways;
}
...
int main(int argc, char **argv) {
    ...
    int32_t minimum = dfs_minimum(eggnog, containers, 0, eggnog);
    // printf("min: %d\n", minimum);
    printf("%d\n", dfs(eggnog, containers, 0, minimum));
