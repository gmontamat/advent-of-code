...
bool play(int32_t player_hp, int32_t player_att, int32_t player_def, int32_t enemy_hp, int32_t enemy_att, int32_t enemy_def) {
    ...
    //if ((double)enemy_hp / (double)damage2enemy <= (double)player_hp / (double)damage2player) return true;
    if (enemy_hp * damage2player <= player_hp * damage2enemy) return true;
    return false;
    // Complex battle loop (slow!)
    // while (1) {
    //     enemy_hp -= MAX(1, player_att - enemy_def);
    //     if (enemy_hp <= 0) return true;
    //     player_hp -= MAX(1, enemy_att - player_def);
...
int main(int argc, char **argv) {
    ...
    const int32_t armor_cst[] = {0, 13, 31, 53, 75, 102};
    const int32_t armor_pts[] = {0, 1, 2, 3, 4, 5};
    const int32_t ring_cst[] = {0, 0, 25, 50, 100, 20, 40, 80};
    const int32_t ring_pts[] = {0, 0, 1, 2, 3, -1, -2, -3};
        ...
        for (size_t j = 0; j < 6; ++j) {
            for (size_t k = 0; k < 7; ++k) {
                for (size_t l = k + 1; l < 8; ++l) {  // never 2 rings of the same type
                    ...
                    if (weapon_cst[i] + armor_cst[j] + ring_cst[k] + ring_cst[l] < min_cost) {
                        if (play(player_hp, player_att, player_def, enemy_hp, enemy_att, enemy_def)) {
                            min_cost = weapon_cst[i] + armor_cst[j] + ring_cst[k] + ring_cst[l];
                        }
                    }
                }
            }
