...
int main(int argc, char **argv) {
    ...
    // Add my feelings and feelings towards me
    for (size_t i=1; i<total_people; ++i) {
        Pair from_to = {0, i};
        Pair to_from = {i, 0};
        hmput(table, from_to, 0);
        hmput(table, to_from, 0);
    }

    // Maximize happiness (minimize happiness) using DFS
    size_t *array = NULL;
    arrput(array, 0);  // set myself as anchor because the table is round!
