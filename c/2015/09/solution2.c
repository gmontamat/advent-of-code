...
uint32_t dfs(DistanceGraph *graph, size_t *array, uint32_t currentDistance, uint32_t maxDistance, size_t totalCities) {
    // printf("{");
    ...
    for (size_t i=0; i<totalCities; ++i) {
        ...
        arrput(arrayNext, i);
        uint32_t newDistance = dfs(graph, arrayNext, nextDistance, maxDistance, totalCities);
...
int main(int argc, char **argv) {
    ...
    // Use DFS to solve
    size_t *array = NULL;
    printf("%d\n", dfs(graph, array, 0, 0, totalCities));
